...
import 'package:todosapp/src/data/mockdata.dart';
...
class LocalStorageTodosApi extends TodosApi {
  ...
  void _init() {
    ...
    if (todosJson != null) {
      final todos = List<Map>.from(json.decode(todosJson) as List)
          .map((jsonMap) => Todo.fromMap(jsonMap))
          .toList();
  ...
  Future<void> saveTodo(Todo todo) {
    List<Todo> todos = [..._todoStreamController.value];
    final todoIndex = todos.indexWhere((t) => t.id == todo.id);
    if (todoIndex >= 0) {
      todos[todoIndex].setTitle = todo.title ;
      todos[todoIndex].setDescription = todo.description;
      todos[todoIndex].setIsCompleted = todo.isCompleted;
    } else {
      todos.add(todo);
    }
  ...
  Future<void> getMockData() {
    final todos = [..._todoStreamController.value];
    if(todos.isEmpty ){
      _todoStreamController.add(lstMockData);
    }
    return _setValue(kTodosCollectionKey, json.encode(lstMockData));
  }
}
